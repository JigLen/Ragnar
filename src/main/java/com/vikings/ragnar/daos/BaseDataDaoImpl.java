...
//                    "where b.imsi = :imsi").setParameter("imsi", imsi);
    ...
    public Collection<?> countNoOfCallFailuresForGivenModel(String model, Date date1, Date date2) {
        Query query = em.createQuery("SELECT ue.manufacturer, base.ueEntity.model, count(*) from BaseDataEntity base join  base.ueEntity as ue where ue.model =:model and base.dateTime between :date1 and :date2");
