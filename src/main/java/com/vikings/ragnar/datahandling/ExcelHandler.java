        ...
        for (Row row : baseData) {
            ...
            try {
                ...
                if (row.getCell(10).getCellType() == Cell.CELL_TYPE_NUMERIC) {
                    baseDataEntity.setImsi(BigDecimal.valueOf(row.getCell(10).getNumericCellValue()));
                } else {
                    baseDataEntity.setImsi((BigDecimal) decimalFormat.parse(row.getCell(10).getStringCellValue()));
                }
                //Col 12 / 14 Hier3_ID
